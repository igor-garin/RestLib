package ru.igarin.base.restlib.network;
...
import ru.igarin.base.restlib.exception.CompulsoryParameterException;
...
public class NetworkConnection {
            ...
            if (Thread.currentThread().getName().equalsIgnoreCase("main")) {
                ...
                Thread thread = new Thread() {
                    ...
                    public void run() {
						...
						try {
							WebView webview = new WebView(context);
							sDefaultUserAgent = webview.getSettings()
									.getUserAgentString();
						} catch (Throwable e) {
							//TODO: catch throwable
							RestLibLog.e(e.getMessage());
						}
                        Looper.loop();
                    }
                };
    ...
    public static class NetworkConnectionResult {
         ...
         * @param result
         */
        public NetworkConnectionResult(final Header[] resultHeader,
                final String result) {
    ...
    public static NetworkConnectionResult retrieveResponseFromService(
            final String url) throws IllegalStateException, IOException,
     ...
     */
    public static NetworkConnectionResult retrieveResponseFromService(
            final String url, final int method) throws IllegalStateException,
     ...
     *            , {@link #METHOD_DELETE}
     * @param parameters
     *            The parameters to add to the request. This is a "key => value"
     *            Map.
     * @return A NetworkConnectionResult containing the response
     * @throws IllegalStateException
     * @throws IOException
     ...
     */
    public static NetworkConnectionResult retrieveResponseFromService(
            final String url, final int method,
            final Map<String, String> parameters) throws IllegalStateException,
            IOException, URISyntaxException, RestClientException {
    ...
    public static NetworkConnectionResult retrieveResponseFromService(
            ...
            final ArrayList<Header> headers) throws IllegalStateException,
            IOException, URISyntaxException, RestClientException {
        return retrieveResponseFromService(url, method, parameters, headers,
                false);
    }
     ...
     *            The headers to add to the request
     * @param isGzipEnabled
     *            Whether we should use gzip compression if available
     * @return A NetworkConnectionResult containing the response
     * @throws IllegalStateException
     * @throws IOException
     * @throws URISyntaxException
     ...
     *            Whether we should use gzip compression if available
     * @param userAgent
     *            The user agent to set in the request. If not given, the
    ...
    public static NetworkConnectionResult retrieveResponseFromService(
            ...
            final Map<String, String> parameters,
            final ArrayList<Header> headers, final boolean isGzipEnabled,
            final String userAgent) throws IllegalStateException, IOException,
            URISyntaxException, RestClientException {

        return retrieveResponseFromService(url, method, parameters, headers,
                isGzipEnabled, userAgent, null);
    }
     ...
     *            default one will be used
     * @param postText
     *            A POSTDATA text that will be added in the request (only if the
     *            method is set to {@link #METHOD_POST})
     * @return A NetworkConnectionResult containing the response
     * @throws IllegalStateException
     * @throws IOException
     * @throws URISyntaxException
    ...
    public static NetworkConnectionResult retrieveResponseFromService(
            ...
            final Map<String, String> parameters,
            final ArrayList<Header> headers, final boolean isGzipEnabled,
            final String userAgent, final byte[] postText)
            throws IllegalStateException, IOException, URISyntaxException,
            RestClientException {
        return retrieveResponseFromService(url, method, parameters, headers,
                isGzipEnabled, userAgent, postText, new ArrayList<String>());
    }

    private static NetworkConnectionResult retrieveResponseFromService(
            final String url, final int method,
            final Map<String, String> parameters,
            final ArrayList<Header> headers, final boolean isGzipEnabled,
            final String userAgent, final byte[] postText,
            final ArrayList<String> previousUrlList)
            throws IllegalStateException, IOException, URISyntaxException,
            RestClientException {
        // Get the request URL
        if (url == null) {
            RestLibLog.e("retrieveResponseFromService - Compulsory Parameter : request URL has not been set");
            throw new CompulsoryParameterException(
                    "Request URL has not been set");
        }
        RestLibLog.d("retrieveResponseFromService - Request url : " + url);

        // Get the request method
        if (method != METHOD_GET && method != METHOD_POST
                && method != METHOD_PUT && method != METHOD_DELETE) {
            RestLibLog.e("retrieveResponseFromService - Request method must be METHOD_GET, METHOD_POST, METHOD_PUT or METHOD_DELETE");
            throw new IllegalArgumentException(
                    "retrieveResponseFromService - Request method must be METHOD_GET, METHOD_POST, METHOD_PUT or METHOD_DELETE");
        }
        RestLibLog.d("retrieveResponseFromService - Request method : " + method);

        // Get the request parameters
        RestLibLog.d("retrieveResponseFromService - Request parameters (number) : "
                + ((parameters != null) ? parameters.size() : ""));

        // Get the request headers
        RestLibLog.d("retrieveResponseFromService - Request headers (number) : "
                + ((headers != null) ? headers.size() : ""));

        // Create the Request
        final AndroidHttpClient client = AndroidHttpClient
                .newInstance(userAgent != null ? userAgent : sDefaultUserAgent);
        RestLibLog.d("retrieveResponseFromService - Request user agent : " + userAgent);
        ...
        try {
            ...
            case METHOD_DELETE: {
                ...
                RestLibLog.i("retrieveResponseFromService - GET Request - complete URL with parameters if any : ");
                final String completeUrl = sb.toString();
                int pos = 0;
                int dumpLength = completeUrl.length();
                while (pos < dumpLength) {
                    RestLibLog.i(completeUrl.substring(pos,
                            Math.min(dumpLength - 1, pos + 120)));
                    pos = pos + 120;
                }
            ...
            case METHOD_POST: {
                ...
                if (parameters != null && !parameters.isEmpty()) {
                    ...
                    for (int i = 0; i < keyListLength; i++) {
                        final String key = keyList.get(i);
                        postRequestParameters.add(new BasicNameValuePair(key,
                                parameters.get(key)));
                    }

                    RestLibLog.i("retrieveResponseFromService - POST Request - parameters list (key => value) : ");

                    final int postRequestParametersLength = postRequestParameters
                            .size();
                    for (int i = 0; i < postRequestParametersLength; i++) {
                        final NameValuePair nameValuePair = postRequestParameters
                                .get(i);
                        RestLibLog.i("- " + nameValuePair.getName() + " => "
                    ...
                    ((HttpPost) request).setEntity(new UrlEncodedFormEntity(
                            postRequestParameters, "UTF-8"));
                                               ...
                                               // for
                    // example)
                    ((HttpPost) request).setEntity(new ByteArrayEntity(postText));
                }
                break;
            }
            default: {
            ...
            RestLibLog.i("retrieveResponseFromService - Request - headers list (name => value) : ");

            final HeaderIterator iterator = request.headerIterator();
            while (iterator.hasNext()) {
                final Header header = iterator.nextHeader();
                RestLibLog.i("- " + header.getName() + " => " + header.getValue());
            }
            ...
            // Launch the request
            String result = null;
            RestLibLog.d("retrieveResponseFromService - Executing the request");
            final HttpResponse response = client.execute(request);

            // Get the response status
            final StatusLine status = response.getStatusLine();
            RestLibLog.d("retrieveResponseFromService - Response status : "
                    + status.getStatusCode());
            final int statusCode = status.getStatusCode();
            if (statusCode != HttpStatus.SC_OK) {
                RestLibLog.e("retrieveResponseFromService - Invalid response from server : "
                        + status.toString());
                if (statusCode == HttpStatus.SC_MOVED_TEMPORARILY) {
                    final Header newLocation = response
                            .getFirstHeader("Location");
                    RestLibLog.i("retrieveResponseFromService - New location : "
                            + newLocation.getValue());
                    throw new RestClientException("New location : "
                            + newLocation, newLocation.getValue());
                } else if (statusCode == HttpStatus.SC_MOVED_PERMANENTLY) {
                    if (method == METHOD_GET) {
                        final String newUrl = response.getHeaders("Location")[0]
                                .getValue();
                        if (!previousUrlList.contains(newUrl)) {
                            RestLibLog.d("retrieveResponseFromService - Url moved permanently - Trying the new url : "
                                    + newUrl);
                            previousUrlList.add(newUrl);
                            return retrieveResponseFromService(newUrl, method,
                            ...
                            throw new RestClientException(
                                    "Moved permanently - Loop detected",
                                    statusCode);
                        }
                    } else {
                        throw new RestClientException(
            ...
            RestLibLog.i("retrieveResponseFromService - Result from webservice : "
                    + result);
     ...
     * @throws IOException
     *             If a problem occurs while reading the InputStream
     */
    private static String convertStreamToString(final InputStream is,
            final boolean isGzipEnabled, final int method,
