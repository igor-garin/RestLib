...
package ru.igarin.base.restlib.network.internal;
...
import ru.igarin.base.restlib.network.UserAgentUtils;
import ru.igarin.base.common.RestLibLog;
import ru.igarin.base.restlib.network.NetworkConnection.ConnectionResult;
import ru.igarin.base.restlib.network.NetworkConnection.Method;
...
public final class NetworkConnectionImpl {
    ...
    public static ConnectionResult execute(Context context, String urlValue, Method method,
        ...
        try {
            ...
            // Log the request
                RestLibLog.d("Request url: " + urlValue);
                RestLibLog.d("Method: " + method.toString());

                if (parameterList != null && !parameterList.isEmpty()) {
                    RestLibLog.d("Parameters:");
                    for (int i = 0, size = parameterList.size(); i < size; i++) {
                        BasicNameValuePair parameter = parameterList.get(i);
                        String message = "- \"" + parameter.getName() + "\" = \""
                                + parameter.getValue() + "\"";
                        RestLibLog.d(message);
                    }

                    RestLibLog.d("Parameters String: \"" + paramBuilder.toString() + "\"");
                }

                if (postText != null) {
                    RestLibLog.d("Post data: " + postText);
                }

                if (headerMap != null && !headerMap.isEmpty()) {
                    RestLibLog.d("Headers:");
                    for (Entry<String, String> header : headerMap.entrySet()) {
                        RestLibLog.d("- " + header.getKey() + " = " + header.getValue());
                    }
                }
            ...
            RestLibLog.d("Response code: " + responseCode);
            ...
            String body = convertStreamToString(connection.getInputStream(),
                ...
                RestLibLog.v("Response body: ");

                int pos = 0;
                int bodyLength = body.length();
                while (pos < bodyLength) {
                    RestLibLog.v(body.substring(pos, Math.min(bodyLength - 1, pos + 200)));
                    pos = pos + 200;
                }
            ...
            RestLibLog.e("KeyManagementException", e);
            throw new ConnectionException(e);
        } catch (NoSuchAlgorithmException e) {
            RestLibLog.e("NoSuchAlgorithmException", e);
            throw new ConnectionException(e);
        } finally {
            if (connection != null) {
