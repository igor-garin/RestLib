...
public class UserManager {
	...
	public static String getSecondUserId(final Context context) {
        final SharedPreferences sharedPrefs = context.getSharedPreferences(SharedPrefsConfig.SHARED_PREFS_FILENAME,
                Context.MODE_PRIVATE);
        String userId = sharedPrefs.getString(SharedPrefsConfig.SHARED_PREFS_SECOND_USER_ID, null);

        if (userId == null) {
            userId = UUID.randomUUID().toString();
            sharedPrefs.edit().putString(SharedPrefsConfig.SHARED_PREFS_SECOND_USER_ID, userId).commit();
        }
